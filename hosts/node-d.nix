...
#    prod-service compromise can't read /run/secrets/rpc_secret (doc 00 §3).
#
...
#    client-side-encrypted ciphertext on the restic/Kopia paths (doc 00 §2/§3).
...
{ ... }:
{
  ...
  fleet = {
    ...
    zone = null; # a gateway has NO zone (doc 01 Phase 3)
    proxyNode = false; # keeps its EXISTING prod proxy duties, set outside fleet scope
    # TODO operator: node-D's ACTUAL tailnet IP, captured in doc 01 Phase 0.
    # node-D is NOT tailscale-proxy-00's target (rsp-asp @ 100.64.0.5) — do
  ...
  #         garage layout apply --version <prev+1>   (doc 01 Phase 3)
}
