...
| Tailscale egress proxy to reach Garage (a **NEW** ExternalName block, e.g. `tailscale-proxy-garage`, pointed at node-D — **never** a repoint of `tailscale-proxy-00`, which serves `rsp-asp` at `100.64.0.5`) | `infrastructure/controllers/staging/tailscale-operator/egress-proxies.yaml` |
