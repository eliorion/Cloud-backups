...
| node-D overlay IP + current prod role | capture the **actual** tailnet IP and existing production role in Phase 0 — node-D is **not** `tailscale-proxy-00`'s target (that is `rsp-asp` at `100.64.0.5`). Do **not** use a `100.100.98.D`-style placeholder or assume it shares the residential-proxy device. |
...
> `100.64.0.5` (`rsp-asp`, the residential scraper-egress proxy) and repointing
