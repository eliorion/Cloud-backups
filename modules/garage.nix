# modules/garage.nix — the Garage object-store service (doc 00 §5, doc 01
...
# never 0.0.0.0 (doc 00 §3/§5 network rule).
#
...
let
  ...
  # Storage nodes may span multiple disks (e.g. NVMe ssd + HDD bulk, doc 03):
...
in
{
  ...
  config = {
    services.garage = {
      enable = true;
      # ⚠️ Design target is v2.3.0 (doc 01 Addressing / Phase 8), but nixpkgs
      ...
      #    add an overlay — see documentations/04-node-a-b-install.md. Renovate
      ...
      settings = {
        ...
        # IDENTICAL on every node or the cluster will not form (doc 00 §5). SINGLE
        # cluster-wide number, NOT additive per node. INTERIM 2-node (A+C) value:
        # node-B is offline long-term and removed from the layout, so the cluster is
        # two zones (onsite A + offsite-2 C). rf=2 = one copy on each — geo-separated
        # durability; W=2 so BOTH must be up to write, R=1 to read. rf cannot exceed
        # the number of distinct zones under maximum zone redundancy: 2 zones now →
        # max rf 2. Garage caches rf in cluster_layout and REFUSES a config/layout rf
        # mismatch, so changing rf means purging cluster_layout + rebuilding the layout
        # (safe here: pool empty, DR data re-pushable). When node-B rejoins (3 zones),
        # purge+rebuild again at rf=3 on ALL nodes, then `garage repair`.
        replication_factor = 2;
        consistency_mode = "consistent"; # rf=2 → W=2 / R=1 (both nodes to write, 1 to read)
        ...
        # or peers cannot reach this node over the tailnet (doc 00 §5).
        #
        ...
        # tailnet (doc 01 Phase 2). Set per host via fleet.bootstrapPeers (each
        ...
        # (doc 00 §3/§9). Use *_file ONLY — inline tokens would render into the
        # world-readable Nix store (doc 00 §5).
        admin = {
          # IPv4 overlay IP → no brackets (see rpc_bind_addr note above).
    ...
    # garage user having none (doc 00 §7, modules/zfs-sanoid.nix).
    users.groups.garage = { };
    users.users.garage = {
      isSystemUser = true;
      group = "garage";
    ...
    # Storage nodes prompt-unlock their encrypted datasets POST-boot (doc 03), so
    ...
    # / `apply --version prev+1` (doc 00 §5, doc 01 Phase 1/2/3), NOT declared
